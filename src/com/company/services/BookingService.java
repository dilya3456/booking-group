...
import com.company.pricing.PricingResult;
...
public class BookingService {
    ...
    public BookingService(IBookingRepository repo, PriceCalculatorService priceCalc, PricingEngine pricingEngine) {
        this.repo = repo;
    ...
    public String createBooking(int passengerId, int flightId, int hotelId, int nights,
        ...
        try {
            ...
            PricingResult pr = pricingEngine.calculate(
                    baseTotal,
                    flight,
                    hotel,
                    nights,
            ...
            repo.insertBookingExtras(con, bookingId, extras, 0.0);

            repo.insertPayment(con, bookingId, total, pm.name());

            repo.insertHistory(con, bookingId, "PRICE_BREAKDOWN", pr.getBreakdown());repo.insertHistory(con, bookingId, "CREATED",
                    "Method=" + pm.name() + ", base=" + round2(baseTotal) + ", total=" + round2(total));
